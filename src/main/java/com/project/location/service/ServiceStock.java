...
public class ServiceStock extends BaseService{
    ...
    public List<Stock> find(String designation,int quantiteMin, int quantiteMax, int locationMin, int locationMax ) throws Exception{
        ...
        if(!Test.testNull(location))arg.add(location);
        List<Stock> reponse = null;
        try{
    ...
    public void insert(Stock stock)throws Exception{
        ...
        try{
            if(stock==null)throw new Exception("stock non initialisé");
            this.hibernateDao.save(stock);           
            if(entree!=null)this.hibernateDao.save(entree);
        }catch(Exception e){
            e.printStackTrace();
    ...
    public void update(Stock stock)throws Exception{
        try{
            this.hibernateDao.update(stock);
        }catch(Exception e){
